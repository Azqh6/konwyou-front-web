    ...
    <div class="container-body message-center" :style="{ width: proxy.globalInfo.bodyWidth + 'px' }">
        ...
        <div class="messgae-panel">
            ...
            <div class="message-list">
                <DataList :loading="loading" :dataSource="messageListInfo" @loadData="loadMessage">
            ...
            <div class="message-item" v-if="data.messageType==1">
                ...
                <div class="message-content">
                   <div>
                    ...
                    对我的文章【<router-link class="a-link" :to="`/post/${data.articleId}`">{{data.articleTitle}}</router-link>】发表了评论
                    <span class="create-time">{{data.createTime}}</span>
                   </div>
             ...
             <div class="message-item" v-if="data.messageType==3">
              ...
              <div class="message-item" v-if="data.messageType==2">
                ...
                <div class="message-content">
                   <div>
                    <router-link class="a-link" :to="`/user/${data.sendUserId}`">@{{data.sendNickName}}</router-link>
                  赞了我的文章【<router-link class="a-link" :to="`/post/${data.articleId}`">{{data.articleTitle}}</router-link>】
                    <span class="create-time">{{data.createTime}}</span>
                   </div>
                </div>
            </div>
              ...
              <div class="message-item" v-if="data.messageType==4">
                ...
                <div class="message-content">
                   <div>
                    <router-link class="a-link" :to="`/user/${data.sendUserId}`">@{{data.sendNickName}}</router-link>
                  在文章【<router-link class="a-link" :to="`/post/${data.articleId}`">{{data.articleTitle}}</router-link>】下载了附件
                    <span class="create-time">{{data.createTime}}</span>
                   </div>
                </div>
            </div>
